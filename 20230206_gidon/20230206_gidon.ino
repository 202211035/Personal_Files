...
void robot_line_trace(int LineType)
{
  ...
  {
    case -4:
            motor_control_r(1, 140); //속도 제어할 것 (-4 | -3 | -2 | -1 | 0 | 1 | 2 | 3 | 4 )
            motor_control_l(1, 10);
            delay(200);
            break;
    
    case -3:
            motor_control_r(1, 120);
            motor_control_l(1, 10);
            delay(100);
            break;
    
    case -2:
            motor_control_r(1, 110);
            motor_control_l(1, 40);
            break;
    ...
    case 2:
            motor_control_r(1, 40);
            motor_control_l(1, 110);
            break;
    
    case 3:
            motor_control_r(1, 10);
            motor_control_l(1, 120);
            delay(100);
            break;
    
    case 4:
            motor_control_r(1, 10);
            motor_control_l(1, 140);
            delay(200);
            break;
...
void loop()
{
  ...
  if(flag == 0)
  {
    ...
    {
     motor_control_r(1, 20);
     motor_control_l(1, 20); //좌우 속도 맞추기
     robot_line_trace(LineType);
    }
    
    if(  (UltrasonicSensorData[2] + UltrasonicSensorData[3] < 150) && (UltrasonicSensorData[2] + UltrasonicSensorData[3] > 0) && (LineType == -5)  )
    {
      flag = 1;
    }
  } 
  else if(flag == 1)
  {
    ...
    if(mode == 1)
    {
      if(UltrasonicSensorData[0] > 75)
      {
        motor_control_l(1, 150);
        motor_control_r(1, 140);
        return 1; //return 삽입 해봄
      }
      else
      {
        robot_stop();
    ...
    if(  (UltrasonicSensorData[2] + UltrasonicSensorData[3] > 150) && (LineType != -5)  )
    {
      flag = 0;
    }
  }
